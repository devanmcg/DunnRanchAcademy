...
Surprisingly, it does not appear that the relative merits of probes vs. beads for wildland fire science have been reported on, although the probe seems to introduce some confusion into what is actually being measured.\footnote[][-2cm]{
...
\section{Soil heating}

Fire effects on soils has received considerable attention from both researchers and managers, and it is not difficult for wildland fire scientists to collect useful data on soil heating during prescribed burns. 
Fire influences the soil either directly, by combustion of organic matter, or indirectly, by exposing soil constituents to heat. 
Soil heating is of interest because it can alter the abundance and chemical composition of nutrients, cause mortality and alter community composition of soil microbes, and when heat doses are particularly high, alter water infiltration and the physical structure of soil particles \footnote{See \citet{pereira2019} and Chapter 6 in \citet{mcgranahan2021}.}.  
In grasslands, specifically, some of the main effects of soil heating include belowground bud dynamics, plant-available nitrogen, and soil microbes \citep[Fig.~\ref{fig:SoilEffects};][]{brady2022, mcgranahan2022}. 

\begin{figure} 
	\begin{center}
		\includegraphics[width=1\textwidth]
			{science/behavior/ChangeN-1}
		\includegraphics[width=1\textwidth]
			{science/behavior/TotMicDiff-1}
	\end{center}
	\caption{Effects of soil heating from \citet{mcgranahan2022}. 
	} \label{fig:SoilEffects}
	%(Fig.~\ref{fig:SoilEffects})
\end{figure} 

In general, soil itself is a poor conductor of energy. 
Only under extreme conditions in the aboveground fire environment, or in the soil itself, will heating have substantial severity on soil properties below the surface. 
Aboveground, large coarse woody debris that remains smoldering on the soil surface can cause heat to move deeper into the soil via conduction. 
Otherwise, flame fronts moving through fine fuels generally do not have long enough residence times to expose soil to substantial heating.
Within the soil, texture and especially moisture content regulate heat transfer.
Generally speaking, higher soil moisture slows heating, as the high specific heat capacity of water makes it resistant to temperature change without substantial heat input. 

Two general approaches to obtaining information on soil heating include \emph{direct measurement} and \emph{modeling}: 

\begin{marginfigure}
	\begin{center}
		\includegraphics[width=2.2in]
		{science/behavior/mod_results-1}
		\caption{An example of soil heating model results published by \citet{mcgranahan2022} showing the temperature of different fires 10 min after exposure at centimeter depth increments.
		The model combines soil surface temperature curve data exactly like those derived in Fig.~\ref{fig:HeatingCurve} with information on soil moisture \& texture and physics-based parameters of fluid dynamics. 
		The initial model was developed by \citet{campbell1994} and applied to estimating heat exposure to prairie grass buds by \citet{choczynska2009}. 
			\label{fig:SoilModResults} } 
		% (Fig.~\ref{fig:SoilModResults})
	\end{center}
\end{marginfigure}
...
\begin{figure} 
	\begin{center}
		\includegraphics[width=1\textwidth]
		{science/behavior/SheFireResults}
	\end{center}
	\caption{Interpolated soil heating curves from the SheFire model \citep{brady2022}. 
	On the left, actual values from the three iButtons at their fixed positions; on the right, heating curves for 10 depth increments, for high and low fuel load scenarios (top and bottom, respectively). 
	} \label{fig:SheFireResults}
	%(Fig.~\ref{fig:SheFireResults})
\end{figure} 
